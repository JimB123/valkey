...
class BgIterationTest : public ::testing::Test {
    ...
    protected:
        ...
        void SetUp() override {
            ...
            EXPECT_CALL(mock, blockInuse_blockClientOnKeys(_,_,_)).WillRepeatedly(Return());
            EXPECT_CALL(mock, blockInuse_unblockClientsOnKey(_)).WillRepeatedly(Return());
        ...
        void expectReadKeyWithUnblock(bgIterator *iter, int itemNum, int unblockItem, const char *value=nullptr) {
            bool blocked = true;
            EXPECT_CALL(mock, blockInuse_unblockClientsOnKey(robjEqualsStr(keyStr(unblockItem))))
        ...
        void simulateBlockedWrite(client *c, int expectedNumberBlockedKeys = 1) {
            EXPECT_CALL(mock, blockInuse_blockClientOnKeys(c,expectedNumberBlockedKeys,_)).Times(1);
        ...
        void simulateUnblockedWrite(client *c) {
            EXPECT_CALL(mock, blockInuse_blockClientOnKeys(c,_,_)).Times(0);
            bool blocked = bgIteration_blockClientIfRequired(c);
            EXPECT_FALSE(blocked);
        }
...
TEST_F(BgIterationTest, earlyTerminationFromMain) {
    ...
    EXPECT_CALL(mock, blockInuse_unblockClientsOnKey(_)).Times(0);
    // We should expect to see unblock called for items 1-4, as they are released from the queue.
    EXPECT_CALL(mock, blockInuse_unblockClientsOnKey(robjEqualsStr(keyStr(1))))
            .WillOnce(Assign(&blocked1, false));
    EXPECT_CALL(mock, blockInuse_unblockClientsOnKey(robjEqualsStr(keyStr(2))))
    ...
    bool blocked0 = true;
    EXPECT_CALL(mock, blockInuse_unblockClientsOnKey(robjEqualsStr(keyStr(0))))
...
TEST_F(BgIterationTest, writeWith3Keys_Replication_NoConsistent_setNewKey_1DependantPast1DependantFuture) {
    ...
    EXPECT_CALL(mock, blockInuse_unblockClientsOnKey(robjEqualsStr(keyStr(13)))).Times(1);
